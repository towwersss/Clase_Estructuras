...
void Curiosity::crear_Mapa(float coeficientedeconectividad) {
    // crea un nuevo grafo y establece las conexiones
        if (mapa != nullptr) {
            delete mapa; // libera la memoria del grafo antiguo
        }
        mapa = new grafo(); // crea un nuevo grafo
        crearConexiones(*mapa, coeficientedeconectividad); // establece las conexiones

        float cantVecinos = 0;
        cout << cantElementos() << endl;
        cout << coeficientedeconectividad << endl;
        cantVecinos = cantElementos() * coeficientedeconectividad;
        cantVecinos = round(cantVecinos);
        cout << cantVecinos << endl;
        organizarAristasPesos(mapa->nodos);
        borrarAristas(mapa->nodos, cantVecinos);
        mapa->imprimirGrafo();
}

void organizarAristasPesos(vector<Nodo*>&nodos){
    for (int i = 0; i < nodos.size();i++ ){
        sort(nodos[i]->aristas.begin(),nodos[i]->aristas.end(),[](Arista* a, Arista* b){
            return a->peso < b->peso;
        });
    }
}

void Curiosity::borrarAristas(vector<Nodo*>nodos,int cantVecinos){
    for(int i=0;i<nodos.size(); i++){
        for (int j = 0; j < (this->cantElementos()-cantVecinos)-1; j++){
            nodos[i]->aristas.pop_back();
        }
    }
}

    
void Curiosity::crearConexiones(grafo& g, float coeficienteDeConectividad){
    vector<Nodo*> nodos;

    // Primero, creamos todos los nodos
    for(auto it = elementos.begin(); it != elementos.end(); it++){
        Nodo* nodo = g.agregarNodo(it->tipoElemento, it->coorDx, it->coorDy);
        nodos.push_back(nodo);
    }

    // Ahora, creamos las aristas entre todos los nodos
    for (auto it1 = nodos.begin(); it1 != nodos.end(); it1++){
        for(auto it2 = it1 + 1; it2 != nodos.end(); it2++){
            g.agregarArista(*it1, *it2, coeficienteDeConectividad);
        }
    }
}

void Curiosity::ruta_mas_larga(grafo* g) {
    g->rutaLargaDFS();
}
